...
async def write_alpaca_training_data(channel: discord.TextChannel, size: int) -> str:
    log.info(f"Fetching images for channel {channel.name} ({channel.id})...")
    messages = fetch_messages(channel, size)
    data = []
    path: Path = Path(f"alpaca_repbot-{size}_messages-{datetime.datetime.now().timestamp()}.json")
